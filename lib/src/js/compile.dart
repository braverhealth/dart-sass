...
NodeCompileResult compile(String path, [CompileOptions? options]) {
  ...
  try {
    var result = compileToResult(path,
        ...
        logger: logger,
        importers: options?.importers?.map((_) => throw ArgumentError(
            'Importers are not supported for compileString().')),
...
NodeCompileResult compileString(String text, [CompileStringOptions? options]) {
  ...
  try {
    var result = compileStringToResult(text,
        ...
        importers: options?.importers?.map((_) => throw ArgumentError(
            'Importers are not supported for compileString().')),
        importer: options?.importer.andThen((_) => throw ArgumentError(
                'Importers are not supported for compileString().')) ??
...
/// See https://github.com/sass/sass/spec/tree/main/js-api/compile.d.ts for
/// details.
JSPromise compileAsync(String path, [CompileOptions? options]) {
  var color = options?.alertColor ?? hasTerminal;
  var ascii = options?.alertAscii ?? glyph.ascii;
  var logger = JSToDartLogger(options?.logger, Logger.stderr(color: color),
      ascii: ascii);
  return (futureToPromise(() async {
    var result = await compileToResultAsync(path,
        ...
        sourceMap: options?.sourceMap ?? false,
        logger: logger,
        importers: options?.importers?.map((importer) => throw ArgumentError(
            'Importers are not supported for compileAsync().')),
...
/// See https://github.com/sass/sass/spec/tree/main/js-api/compile.d.ts for
/// details.
JSPromise compileStringAsync(String text, [CompileStringOptions? options]) {
  var color = options?.alertColor ?? hasTerminal;
  var ascii = options?.alertAscii ?? glyph.ascii;
  var logger = JSToDartLogger(options?.logger, Logger.stderr(color: color),
      ascii: ascii);
  return futureToPromise(() async {
    var result = await compileStringToResultAsync(text,
        ...
        logger: logger,
        importers: options?.importers?.map((importer) => throw ArgumentError(
            'Importers are not supported for compileStringAsync().')),
...
OutputStyle _parseOutputStyle(String? style) => switch (style) {
      ...
      _ => jsThrow(Exception('Unknown output style "$style".'))
    };
